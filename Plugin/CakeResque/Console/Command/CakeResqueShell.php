 ...
 */
	public function start($args = null, $scheduler = false) {
		...
		$cmd = implode(' ', array(
			...
			sprintf("processLimit=%s \\\n", escapeshellarg( $processLimit )),
 ...
 */
	public function stats() {
		...
		$failed = 0;
		// foreach ($workers as $worker) {
		// 	// pr( $worker->getStat('processed') );
		// 	pr( $worker );
		// 	pr( $worker->getStat('failed') );
		// 	pr( $worker->getStat('processed') );
		// 	pr( call_user_func(CakeResqueShell::$cakeResque . '::getWorkerStartDate', $worker) );
		// 	pr( Resque_Stat::get() );
		// 	exit;
		// }
		// // pr( $workers );
