...
class Resque_Worker
{
    ...
    public function work($interval = 5)
    {
        ...
        while (true) {
            $workers = $this->all();
            $countOfWorkers = count( $workers ); 
            foreach( $workers as $worker) {
                // removing the inactive worker such that it should not be the part of actual count of process.
                $id = (string)$worker;
                if( $worker->getStat('failed') == 1 ){
                    pr( "worker failure status of ". $id . " is " . $worker->getStat('failed') );
                    $countOfWorkers--;    
                }
            }
            pr( "Total active workers: " . $countOfWorkers );
            if( !empty( $this->workerCountz ) && $countOfWorkers >= $this->workerCountz ){
                pr( "Max number of requests we can have in system:" . $this->workerCountz );
                pr( "not able to consume as there are 2 mdm workers" );
                $this->paused = true;
            }else{
                pr( "consuming from queue assuming criteria matched: " . $countOfWorkers );
                $this->paused = false;
            }
            if ($this->shutdown) {
                break;
...
pr( "ended at: " . date( 'Y-m-d H:i:s' ) );
    ...
    {
        $this->log(array('message' => 'Starting worker1 ' . $this, 'data' => array('type' => 'start', 'worker' => (string) $this)), self::LOG_TYPE_INFO);
